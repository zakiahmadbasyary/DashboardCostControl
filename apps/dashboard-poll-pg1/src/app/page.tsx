...
import { Truck, LayoutGrid, ShieldCheck, Menu, X, Wrench } from "lucide-react";

export default function DashboardPollPg1Page() {
  ...
  return (
    <div className="min-h-screen bg-[#F7F9F7] text-[#17231B] flex flex-col justify-between font-sans selection:bg-[#16823B] selection:text-white">
      {/* Dedicated Header for Dashboard Poll PG 1 */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="h-16 sm:h-18 flex items-center justify-between gap-4">
            ...
            <nav className="hidden lg:flex items-center gap-0.5 xl:gap-1.5 shrink-0 whitespace-nowrap overflow-hidden py-1">
              {navItems.map((item) => {
                const isActive = item.key === "poll_pg1";
      ...
      <main className="max-w-4xl mx-auto my-12 text-center space-y-6 px-4 flex-1">
        ...
        <h2 className="text-3xl font-extrabold text-[#17231B]">Dashboard Poll PG 1 (Armada & Transportasi)</h2>
        <p className="text-[#5F6B63] text-sm leading-relaxed max-w-xl mx-auto">
          Pengendalian biaya operasional pool armada PG1, efisiensi konsumsi bahan bakar, dan utilitas kendaraan.
        </p>
      </main>

      <footer className="text-center text-xs text-[#5F6B63] border-t border-[#DDE5DF] bg-white py-6">
        © 2026 GGF AgroMetric Platform — Dashboard Poll PG 1 Workspace
      </footer>
    </div>
  );
}
