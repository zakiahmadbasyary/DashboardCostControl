...
import { Landmark, ShieldCheck, LayoutGrid, Menu, X } from "lucide-react";
...
export default function DashboardCPage() {
  ...
  const navItems = [
    { key: "wip", label: "WIP ACC", url: wipUrl },
    { key: "dashboard_a", label: "WIP PG1", url: dashboardAUrl },
    { key: "dashboard_b", label: "HPP PG1", url: dashboardBUrl },
    { key: "dashboard_c", label: "HPP M3", url: dashboardCUrl, active: true },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#F7F9F7] text-[#17231B] flex flex-col justify-between font-sans selection:bg-[#16823B] selection:text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="h-16 sm:h-18 flex items-center justify-between gap-4">
            <a href={portalUrl} className="flex items-center gap-3 shrink-0 py-1 group focus:outline-none">
              ...
              <div>
                <span className="font-extrabold text-sm sm:text-base text-[#17231B] group-hover:text-[#16823B] tracking-tight leading-none transition-colors block">
                  Dashboard HPP M3
                </span>
                <span className="text-[10px] sm:text-xs text-[#5F6B63] hidden md:block mt-0.5 font-medium">
                  Audit Finansial & Laporan Eksekutif
                </span>
              </div>
            </a>
      ...
      <main className="max-w-4xl mx-auto my-12 text-center space-y-6 px-4 flex-1">
        ...
        <h2 className="text-3xl font-extrabold text-[#17231B]">Dashboard HPP M3 (Audit Finansial)</h2>
        <p className="text-[#5F6B63] text-sm leading-relaxed max-w-xl mx-auto">
          Aplikasi ini disiapkan untuk modul Audit Finansial & Laporan Eksekutif fasilitas M3. Menggunakan database terpisah dan mengonsumsi header lokal independen.
        </p>
      </main>

      <footer className="text-center text-xs text-[#5F6B63] border-t border-[#DDE5DF] bg-white py-6">
        © 2026 GGF AgroMetric Platform — Dashboard HPP M3 Workspace
      </footer>
    </div>
  );
}
